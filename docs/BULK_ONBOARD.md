...
> - 申請／開戶批核後的**初始臨時密碼一律預設為 `1234`**；首次登入 APP 會強制要求設定 **4-8 位**的新私人密碼後才可使用（v5.7 統一密碼規則）。
...
- 已存在的 YMIS 會被跳過（不覆蓋）。
- **v5.3.2：YMIS／Email 全團唯一（包括已停用帳號）**——同一個 YMIS／Email 不可開第二個帳號；若該編號屬於已停用帳號，會提示到「用戶管理 → 🔄 重新啟用」處理，該列會被拒（計入失敗），不會重複開戶。
- 領袖帳號（role≠member）不會出現在進度名單；純成員會。
- 「重設密碼（可直接設定新密碼）/ 改資料 / 停用／重新啟用 / 刪除成員 / 操作紀錄 / 審批歷史」同樣在 APP「用戶管理」頁完成。

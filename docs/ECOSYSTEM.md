...
| 變數 | 用途 | 必需 |
|---|---|---|
| `SUPER_KEY` | APP ADMIN 設定嘅管理 key（= 維護帳戶密碼；**只存在 Vercel 功能變數，leaf GS 冇**）：① 保護 `/api/register` ② 超管登入時由 `api/super-login.js` 比對，之後用 leaf apikey 簽 sig（`action=superLogin`）俾 leaf 驗 | 管理 API／超管登入必需 |
